...
- **Location**: [skills/project-management/SKILL.md](skills/project-management/SKILL.md)

### 2. Development (`development`)
Drives all core coding tasks: planning, implementation, bug fixing, structured debugging, refactoring, and safe removals.
- **Location**: [skills/development/SKILL.md](skills/development/SKILL.md)
...
- **Location**: [skills/testing/SKILL.md](skills/testing/SKILL.md)
...
- **Location**: [skills/git-operations/SKILL.md](skills/git-operations/SKILL.md)
...
- **Location**: [skills/project-memory/SKILL.md](skills/project-memory/SKILL.md)
...
|---|---|---|---|:---:|
| **Project Management** | `/init` | [init.md](workflows/init.md) | Audit structure, detect tech stack, bootstrap memory | ✅ Yes |
| **Development** | `/feature` | [feature.md](workflows/feature.md) | Plan & implement a new feature following conventions | ✅ Yes |
| | `/fix` | [fix.md](workflows/fix.md) | Diagnose and fix a bug with root-cause analysis | ✅ Yes |
| | `/debug` | [debug.md](workflows/debug.md) | Investigate issues (reproduce, isolate, report) | ⚠️ Conditional |
| | `/refactor` | [refactor.md](workflows/refactor.md) | Restructure code preserving external behavior | ✅ Yes |
| | `/modify` | [modify.md](workflows/modify.md) | Make targeted code modification with minimal scope | ⚠️ Conditional |
| | `/remove` | [remove.md](workflows/remove.md) | Safely remove code/features with dependency check | ✅ Yes |
| **Code Review** | `/review` | [review.md](workflows/review.md) | Review quality, readability, and correctness | ❌ Read-only |
| | `/review-security` | [review-security.md](workflows/review-security.md) | Security review (XSS, injection, auth, secrets) | ❌ Read-only |
| | `/review-performance` | [review-performance.md](workflows/review-performance.md) | Performance review (bottlenecks, memory, complexity) | ❌ Read-only |
| | `/review-architecture` | [review-architecture.md](workflows/review-architecture.md) | Architecture review (SOLID, coupling, patterns) | ❌ Read-only |
| **Testing** | `/test` | [test.md](workflows/test.md) | Run full test suite and report results | ❌ Read-only |
| | `/test-unit` | [test-unit.md](workflows/test-unit.md) | Create or run unit tests | ⚠️ Conditional |
| | `/test-integration` | [test-integration.md](workflows/test-integration.md) | Create or run integration tests | ⚠️ Conditional |
| | `/test-e2e` | [test-e2e.md](workflows/test-e2e.md) | Create or run end-to-end user flow tests | ⚠️ Conditional |
| | `/test-fix` | [test-fix.md](workflows/test-fix.md) | Diagnose and fix failing tests | ✅ Yes |
| **Git Operations** | `/git-status` | [git-status.md](workflows/git-status.md) | Show working tree status & staged changes | ❌ Read-only |
| | `/git-diff` | [git-diff.md](workflows/git-diff.md) | Show diffs (working, staged, or between refs) | ❌ Read-only |
| | `/git-commit` | [git-commit.md](workflows/git-commit.md) | Stage & commit with conventional message | ✅ Yes |
| | `/git-branch` | [git-branch.md](workflows/git-branch.md) | Create, switch, list, or delete branches | ❌ Read-only |
| | `/git-log` | [git-log.md](workflows/git-log.md) | Show recent commit history graph | ❌ Read-only |
| | `/git-undo` | [git-undo.md](workflows/git-undo.md) | Soft/hard reset, unstage, or discard changes | ⚠️ Conditional |
| | `/git-sync` | [git-sync.md](workflows/git-sync.md) | Pull/push with conflict detection | ❌ Read-only |
| **Project Memory** | `/memory` | [memory.md](workflows/memory.md) | Show current memory summary | ❌ Read-only |
| | `/memory-init` | [memory-init.md](workflows/memory-init.md) | Initialize memory by scanning codebase | ✅ Yes |
| | `/memory-update` | [memory-update.md](workflows/memory-update.md) | Targeted update to memory categories | ✅ Yes |
| | `/memory-refresh` | [memory-refresh.md](workflows/memory-refresh.md) | Re-scan codebase and reconcile memory | ✅ Yes |
| | `/memory-search` | [memory-search.md](workflows/memory-search.md) | Keyword search across memory documents | ❌ Read-only |
| | `/memory-forget` | [memory-forget.md](workflows/memory-forget.md) | Soft-delete/archive an entry from memory | ✅ Yes |
| | `/memory-export` | [memory-export.md](workflows/memory-export.md) | Export memory as consolidated markdown/folder | ❌ Read-only |
